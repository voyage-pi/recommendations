...
def create_route_on_itinerary(itineraries :List[TripItinerary])->TripItinerary:
    ...
    for iter in itineraries:
        ...
        for i,d in enumerate(days):
            ...
            new_day=DayItineraryRoute(routes=polylines_duration_list,morning_activities=d.morning_activities,afternoon_activities=d.afternoon_activities,date=d.date)
            new_days.append(new_day)  
        # here for future work make a system of ranking metrics to evaluate given itineraries 
        choosen_routed_itinerary=TripItinerary(id=iter.id,start_date=iter.start_date,end_date=iter.end_date,days=new_days)
